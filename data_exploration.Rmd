...
hubei_v_others = province_count %>%
  mutate(isHubei = ifelse(provinceName == '湖北省', 'Hubei', 'Others')) %>%
...
hubei_v_others %>% 
  ...
  ggplot(aes(x = date_num, y = tot_count, col = isHubei, group = isHubei)) + 
  geom_point() +
  geom_line() +
  xlab('Date') + 
  scale_y_continuous('Total Count') +
  ggtitle('Daily Confirmed and Suspected') +
  scale_color_discrete('', breaks=c('Hubei', 'Others')) +
